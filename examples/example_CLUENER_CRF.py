...
from lightning_ie.nn import ConditionalRandomField
from lightning_ie.util import SequenceDataset, sequence_padding, read_jsonl, write_jsonl, decode_bio_seq
...
@dataclass()
class CLUENERDataModuleArguments:
...
class CLUENERDataModule(pl.LightningDataModule):

    def __init__(self, data, tokenizer, **kwargs):
        super(CLUENERDataModule, self).__init__()
    ...
    def encode_example(self, offset_mapping, example):
        ...
        bio_seq = [schema2id['O']] + [schema2id['O']] * (seq_len - mask_len - 1) + [schema2id['O']] * mask_len
...
class Module(pl.LightningModule):
    def __init__(self, plm_model, tokenizer, **kwargs):
        ...
        self.crf = ConditionalRandomField(len(schema2id))
    ...
    def forward(self,
        ...
        hidden = self.linear(plm_hidden)
        output = {}

        if labels:
            loss_bio = self.crf.neg_log_likelihood_loss(hidden, labels['bio_seq'], attention_mask)
            output['loss'] = {'bio_loss'  : loss_bio,
                              'total_loss': loss_bio}
            return output

        bio_seq = self.crf(hidden, attention_mask)
        output['bio_seq'] = bio_seq
        return output
    ...
    def predict_step(self, batch, batch_idx: int = 0, dataloader_idx: int = 0):
        ...
        batch_bio_seq = outputs['bio_seq']

        cls_mask = batch.input_ids == self.tokenizer.cls_token_id
        for i, j in zip(*torch.where(cls_mask)):
            batch_bio_seq[i][j] = schema2id['O']

        sep_mask = batch.input_ids == self.tokenizer.sep_token_id
        for i, j in zip(*torch.where(sep_mask)):
            batch_bio_seq[i][j] = schema2id['O']
...
def main(module_args, data_module_args, trainer_args):
    ...
    dataset = CLUENERDataModule({'train': train_data, 'valid': valid_data, 'test': test_data},
...
if __name__ == '__main__':
    ...
    parser = HfArgumentParser([ModuleArguments, CLUENERDataModuleArguments])
